...
fun main() {
  ...
  val hrefGraph: Graph<URI, DefaultEdge> = createHrefGraph()

  // find the vertex corresponding to www.jgrapht.org
  val start: URI = hrefGraph
    .vertexSet()
    .stream()
    .filter { uri -> uri.host.equals("www.jgrapht.org") }
    .findAny()
    .get()
...
private fun createHrefGraph(): Graph<URI, DefaultEdge> {
  val g: Graph<URI, DefaultEdge> = DefaultDirectedGraph(DefaultEdge::class.java)
...
private fun renderHrefGraph(hrefGraph: Graph<URI, DefaultEdge>) {
  val exporter: DOTExporter<URI, DefaultEdge> = DOTExporter { v ->
    v.host.replace('.', '_')
  }
